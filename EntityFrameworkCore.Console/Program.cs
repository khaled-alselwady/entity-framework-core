...
{
    await context.Coaches.Where(c => c.Name.StartsWith("Jose")).ExecuteDeleteAsync();
}
async Task ExecuteUpdate()
{
    await context.Coaches.Where(c => c.Name.EndsWith("Mohammed"))
    .ExecuteUpdateAsync(set => set
    .SetProperty(c => c.Name, "Waleed Mohammed")
    .SetProperty(c => c.CreatedDate, DateTime.Now)
    );
}
